...
DECLARE_MESSAGE( m_Scoreboard, TeamNames );
DECLARE_MESSAGE( m_Scoreboard, TeamScores );
...
int CHudScoreboard :: DrawPlayers( int xpos_rel, float list_slot, int nameoffset, int team )
{
	...
	while ( 1 )
	{
		...
		for ( int i = 1; i < MAX_PLAYERS; i++ )
		{
			...
			{
				if( team == g_PlayerExtraInfo[i].teamnumber )  // make sure it is the specified team
				{
...
int CHudScoreboard :: MsgFunc_TeamNames( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	...
	for (int i = 0; i < m_iNumTeams; i++)
	{
		...
		gHUD.m_TextMessage.LocaliseTextString( READ_STRING(), g_TeamInfo[i+1].name, MAX_TEAMNAME_SIZE );
...
int CHudScoreboard :: MsgFunc_TeamScores( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
