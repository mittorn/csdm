...
void CBasePlayerWeapon :: DefaultFire( CBasePlayer *m_pPlayer, int cShots, wepspread_t spread, float damage, int anim, char *sound, int genevent, int RadOfBright )
{
	...
	Vector vecDir( m_pPlayer->FireBulletsPlayer( cShots, vecSrc, vecAim, vecAcc, 8192,0, 0, damage, m_pPlayer->pev, m_pPlayer->random_seed ) );
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
	...
	if ( g_runfuncs && ( HUD_GetWeaponAnim() != to->client.weaponanim ) || to->client.viewmodel != player.pev->viewmodel )
	{
		int body = 2;
