...
/*
void DrawSmoke( float *attachment )
{
	return;
	vec3_t angles, forward, right, up;
	AngleVectors( angles, forward, up, right );

	int  smoke = gEngfuncs.pEventAPI->EV_FindModelIndex ( "sprites/smoke.spr" );

	for( int i = 1; i <= 20; i++ )
	{
		TEMPENTITY *pTemp = gEngfuncs.pEfxAPI->R_TempSprite( attachment,
			Vector( gEngfuncs.pfnRandomLong( -30, 30 ), gEngfuncs.pfnRandomLong( -30, 30 ), gEngfuncs.pfnRandomLong( -30, 30 ) ),
			0.3, smoke, kRenderTransAdd, kRenderFxNone, 1.0, 0.5, FTENT_SPRANIMATE| FTENT_FADEOUT );

		if(pTemp)
		{
			pTemp->fadeSpeed = 10.0;
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	case 5001:
		...
		//DrawSmoke( (float *)&entity->attachment[0] );
		break;
	case 5011:
		gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[1], atoi( event->options) );
		//DrawSmoke( (float *)&entity->attachment[1] );
		break;
	case 5021:
		gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[2], atoi( event->options) );
		//DrawSmoke( (float *)&entity->attachment[2] );
		break;
	case 5031:
		gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[3], atoi( event->options) );
		//DrawSmoke( (float *)&entity->attachment[3] );
		break;
	case 5002:
