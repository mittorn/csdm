...
Vector CBaseEntity::FireBulletsPlayer2 ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	return 0;
}
...
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
    ...
    if(iBulletType!=14)
        iShot = RANDOM_LONG(1,4);
    else
        iShot = RANDOM_LONG(12,20);
    ...
    vecEnd = vecSrc + vecDir * flNewDistance;
    ...
    while(newDamage > 0)
    {
        ...
        if (tr.flFraction != 1.0)
            ...
            if ( iDamage )
                {
                    pEntity->TraceAttack(pevAttacker, newDamage, vecDir, &tr, DMG_BULLET | ((newDamage > 16) ? DMG_ALWAYSGIB : DMG_NEVERGIB) );
                DecalGunshot(&tr,0);
                TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
            }
            else switch(iBulletType)
            {
                    default:
                        pEntity->TraceAttack(pevAttacker, newDamage, vecDir, &tr, DMG_BULLET);
                    break;
            ...
            vecSrc = vecSrc - tr.vecEndPos;
            flNewDistance -= vecSrc.Length();
            
            newDamage /= 4;
            
            if(iDamage > 20)
                vecEnd = tr.vecEndPos + vecDir * (newDamage * RANDOM_FLOAT(1.5,2.2));
            else if(iDamage > 25)
                vecEnd = tr.vecEndPos + vecDir * (newDamage * RANDOM_FLOAT(1.5,2.5));
            else if(iDamage > 30)
                vecEnd = tr.vecEndPos + vecDir * (newDamage * RANDOM_FLOAT(2,3));
            else if(iDamage > 40)
                vecEnd = tr.vecEndPos + vecDir * (newDamage * 4);
            
            UTIL_TraceLine(tr.vecEndPos + vecDir, vecEnd, dont_ignore_monsters, ENT(pev), &tr);
