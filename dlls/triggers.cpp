...
//============ func_bomb_target ==================
int NumBombPlants = 1;
class CFuncBombTarget : public CBaseTrigger
{
public:
    void KeyValue( KeyValueData *pkvd );
    void Spawn( void );
...
LINK_ENTITY_TO_CLASS( func_bomb_target, CFuncBombTarget ); //link func_jail to CFuncJail

void CFuncBombTarget :: KeyValue( KeyValueData *pkvd )
{
    CBaseTrigger::KeyValue( pkvd );
}
void CFuncBombTarget :: Precache( void )
{
    pev->solid = SOLID_NOT;
	if( NumBombPlants == 1 )
	{
		pev->skin = CONTENT_BOMBPLANT1;
		NumBombPlants = 2;
	}
	else
	{
		pev->skin = CONTENT_BOMBPLANT2;
		NumBombPlants = 1;
	}
...
void CFuncBombTarget::Spawn( void )
{
    Precache();
