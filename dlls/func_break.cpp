...
void CBreakable::Restart()
{
	...
	pev->takedamage = hack_takedamage;

	pev->playerclass = hack_playerclass;

	pev->flags = hack_flags;
	pev->solid		= SOLID_BSP;
    pev->movetype	= MOVETYPE_PUSH;

	SET_MODEL(ENT(pev), STRING(pev->model) );//set size and link into world.

	pev->rendermode = hack_rendermode;
	pev->renderamt = hack_renderamt;
	pev->targetname   = m_flLastTargetName;
	pev->target       = m_strCurrentTarget;
...
void CBreakable::Spawn( void )
{
	...
	hack_rendermode = pev->rendermode;
	hack_renderamt = pev->renderamt;
	hack_flags = pev->flags;
	hack_angles = m_angle;
	hack_takedamage = pev->takedamage;
	hack_playerclass = pev->playerclass;
	hack_health = pev->health;
